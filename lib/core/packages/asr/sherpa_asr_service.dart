...
class SherpaAsrService implements AsrService {
  ...
  @override
  bool get isInitialized => _offlineRecognizer != null;
  ...
  Future<void> initialize(AsrModelEntity model, String modelPath) async {
    // Инициализируем bindings один раз
    ...
    // Создаём offline recognizer
    _offlineRecognizer = _createOfflineRecognizer(config);

    // Создаём online recognizer если модель поддерживает streaming
  ...
  Future<AsrResult> transcribeFile(String filePath) async {
    ...
    try {
      ...
      throw AsrProcessingException('Failed to transcribe file: $e', e);
    }
  }
